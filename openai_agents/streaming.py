...
# def handle_billing_stream(event_dict) -> None:
#     """
#     This function handles the 'Thinking' process of the Billing Agent.
#     We indent it to show it is a Sub-Process.
#     """
#     event = event_dict.get("event")
    
#     # 1. Text Generation (The sub-agent is thinking/speaking)
#     if event.type == "message_delta" and event.delta.content:
#         # Print indented, colored output (using ANSI codes for Grey)
#         print(f"\033[90m  [Billing Agent]: {event.delta.content}\033[0m", end="", flush=True)

#     # 2. Tool Calls (The sub-agent is using the Python function)
#     elif event.type == "run_step_delta" and event.delta.step_details:
#         if event.delta.step_details.type == "tool_calls":
#             print(f"\n\033[90m  [Billing Agent] 🛠️  Checking Database...\033[0m", end="", flush=True)
